...
class PhotoPickerHelper private constructor(
    ...
    companion object {
        private const val TAG = "PhotoPickerHelper"
    ...
    fun launchCamera(customUri: Uri? = null) {
        runCatching {
            ...
            if (launcher == null) {
                Log.w(TAG, "launchCamera failed: TakePicture launcher is not registered")
                onResult?.invoke(emptyList())
                return
            }
            launcher.launch(uri)
        }.onFailure {
            Log.e(TAG, "launchCamera failed", it)
            cameraUri = null
            onResult?.invoke(emptyList())
        }
    }
    ...
    fun launchVideoCamera(customUri: Uri? = null) {
        runCatching {
            ...
            this.cameraUri = uri
            val launcher = recordVideoLauncher
            if (launcher == null) {
                Log.w(TAG, "launchVideoCamera failed: CaptureVideo launcher is not registered")
                onResult?.invoke(emptyList())
                return
            }
            launcher.launch(uri)
        }.onFailure {
            Log.e(TAG, "launchVideoCamera failed", it)
            cameraUri = null
            onResult?.invoke(emptyList())
        }
    }
    ...
    private fun createTempUri(fileName: String): Uri {
        val pickerDir = java.io.File(context.cacheDir, "picker")
        if (!pickerDir.exists()) {
            pickerDir.mkdirs()
        }
